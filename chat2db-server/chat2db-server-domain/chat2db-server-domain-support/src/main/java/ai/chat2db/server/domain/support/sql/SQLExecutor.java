 ...
 */
package ai.chat2db.server.domain.support.sql;
...
import static ai.chat2db.server.domain.support.util.ResultSetUtils.buildColumn;
import static ai.chat2db.server.domain.support.util.ResultSetUtils.buildFunction;
import static ai.chat2db.server.domain.support.util.ResultSetUtils.buildProcedure;
import static ai.chat2db.server.domain.support.util.ResultSetUtils.buildTable;
import static ai.chat2db.server.domain.support.util.ResultSetUtils.buildTableIndexColumn;
...
public class SQLExecutor {
                ...
                try {
                    ...
                    for (int i = 1; i <= col; i++) {
                        headerList.add(Header.builder()
                            .dataType(ai.chat2db.server.domain.support.util.JdbcUtils.resolveDataType(
                    ...
                    while (rs.next()) {
                        ...
                        dataList.add(row);
                        for (int i = 1; i <= col; i++) {
                            row.add(ai.chat2db.server.domain.support.util.JdbcUtils.getResultSetValue(rs, i));
                        }
                    }
     ...
     * @return
     */
    public List<ai.chat2db.server.domain.support.model.Function> functions(String databaseName,
        String schemaName) {
        List<ai.chat2db.server.domain.support.model.Function> functions = Lists.newArrayList();
        try {
