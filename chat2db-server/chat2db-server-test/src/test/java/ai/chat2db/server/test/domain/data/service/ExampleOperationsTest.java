...
public class ExampleOperationsTest extends BaseTest {
    ...
    public void example() {
        for (DialectProperties dialectProperties : dialectPropertiesList) {
            String createTable = tableService.createTableExample(dialectProperties.getDbType());
            log.info("返回建表语句:{}", createTable);
            Assertions.assertNotNull(createTable, "查询样例失败");
            String alterTable = tableService.alterTableExample(dialectProperties.getDbType());
